...
module END.Example.FinnishRandonneur (
    -- * Finnish Randonneur
...
data FinnRandoSolution = FinnRandoSolution City [City] [City]
  deriving (Eq, Ord)

instance Show FinnRandoSolution where
    showsPrec d = showsPrec d . solutionToList

instance NFData FinnRandoSolution where
    rnf (FinnRandoSolution x ys zs) = rnf x `seq` rnf ys `seq` rnf zs

solutionToList :: FinnRandoSolution -> [City]
solutionToList (FinnRandoSolution x ys zs) = x : ys ++ reverse zs

finnishRandonneurProblem
    :: Double       -- ^ cut off distance, stop if shorter route found.
    -> Set.Set City -- ^ cities to visit, assumed to be non-empty.
    -> Problem Double FinnRandoSolution
finnishRandonneurProblem cutoff cities = Problem {..} where
    fitness :: FinnRandoSolution -> Double
    fitness xs = negate $ L.foldl' (+) 0 $ map (uncurry cityDistance) $ pairs $ solutionToList xs

    addStep c (FinnRandoSolution x ys zs)
        | length ys < length zs  = FinnRandoSolution x (ys ++ [c]) zs
        | otherwise              = FinnRandoSolution x ys          (zs ++ [c])

    completeSolution :: SM.SMGen -> FinnRandoSolution -> FinnRandoSolution
    completeSolution g0 sol0 =
        go g0 sol0 (Set.difference cities (Set.fromList (solutionToList sol0)))
      where
        go :: SM.SMGen -> FinnRandoSolution -> Set.Set City -> FinnRandoSolution
        go !g sol !rest
            | Set.null rest = sol
            | otherwise     =
                let (w, g') = SM.bitmaskWithRejection64 (fromIntegral (Set.size rest)) g
                    c       = Set.elemAt (fromIntegral w) rest
                in go g' (addStep c sol) (Set.delete c rest)

    commitment n (FinnRandoSolution x ys zs)
        | listToMaybe ys' < listToMaybe zs' = FinnRandoSolution x ys' zs'
        | otherwise                         = FinnRandoSolution x zs' ys'
      where
        ys' = take n ys
        zs' = take n zs

    -- we try to make a loop, so let's pick on
    initialSolution :: FinnRandoSolution
    initialSolution = FinnRandoSolution
        (Set.findMin cities) [] []

    initialCommitment = 1

    -- zero loop would be great, but is impossible :(
    maximumFitness  = negate cutoff
